...
interface HydrogenStorageChartProps {
  onHourHover?: (hourIdx: number, values: { key: StrategyKey; val: number }[]) => void
}

export default function HydrogenStorageChart({ onHourHover }: HydrogenStorageChartProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  const { dataset, selectedStrategies } = useStrategy()
  const metricData = dataset?.H_HS as Record<StrategyKey, number[]>
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          const hourIdx = Number.parseInt(String(arr[0]?.axisValue ?? '1').replace('h', ''), 10) - 1
          const vals = arr.map(p => ({
            key: p.seriesName.replace('H_HS_', '') as StrategyKey,
            val: p.value,
          }))
          ...
          let s = `<div style="font-family:'Rajdhani',sans-serif;font-size:13px;color:#00d4ff;margin-bottom:6px;letter-spacing:1px">第 ${arr[0]?.axisValue ?? '1'} 小时</div>`
          arr.forEach(p => {
            ...
            s += `<div style="display:flex;align-items:center;gap:8px;padding:2px 0">
              ...
              <span style="font-weight:700">${p.value.toFixed(3)}</span>
              <span style="color:${delta >= 0 ? '#ff7043' : '#69f0ae'};font-size:10px">${sign}${delta.toFixed(3)}</span>
            </div>`
          })
          return s
        },
  ...
  }, [dataset, metricData, selectedStrategies])

  useEChart(containerRef, option, ['H_HS'])
