...
![EcoClaw（碳智调度官）自主执行架构图](docs/figures/ecoclaw-architecture.png)

图 1 EcoClaw（碳智调度官）自主执行架构图。EcoClaw 既响应用户指令，也在后台持续值守，通过自然语言调度与自主监控双引擎实现园区节能减排场景下的闭环决策执行。可编辑母版文件位于 `docs/figures/ecoclaw-architecture.pptx`。
